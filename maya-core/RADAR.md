...
| Pre-Dispatch Crush Light Review Closure | `adopted` | `chat_derived`, `cross_combined` | Verification / Pre-dispatch focus stability | Lokal als enger Review- und Evidence-Block übernommen: `Pre-Dispatch Crush Light` wird jetzt über fokusnahe Antwortlagen darauf beobachtet, dass die letzte User-Nachricht gegenüber älteren Breitenpfaden priorisiert bleibt, dass innerhalb der letzten Nachricht request-nahe Segmente vor bloßem Lead-in gewinnen, dass der Fallback auf den eigentlichen Druck auch ohne Request-Phrase weiter trägt und dass driftige absolute Zeitbehauptungen trotz enger Fokusmarkierung weiter von der Guardrail-Nachspur markiert werden | `__tests__/lib/maya-pre-dispatch-crush-light-review.test.ts`, `lib/maya-provider-dispatch.ts`, `lib/maya-thread-digest.ts`, `lib/maya-epistemic-guardrail.ts`, `STATE.md`, `docs/spec-packs/products/maya/STATUS.md` |
| Guardrail Signal Stability Review Closure | `adopted` | `chat_derived`, `cross_combined` | Verification / Guardrail stability | Lokal als enger Review- und Evidence-Block übernommen: `mirror`, `overclaimWarning` und `freshnessWarning` werden jetzt über guardrail-nahe Antwortlagen auf ihr derzeitiges Schwellenverhalten beobachtet, also darauf, dass repo-geerdete Antworten mit Frischeanker ruhig bleiben, driftige absolute Zeitbehauptungen weiter beide Warnsignale auslösen und repo-kontextnahe Frischeformulierungen ohne expliziten Commit-Anker aktuell noch still bleiben; der Block dokumentiert damit die aktuelle Heuristik, statt still eine neue Guardrail-Policy zu öffnen | `__tests__/lib/maya-guardrail-signal-stability-review.test.ts`, `lib/maya-epistemic-guardrail.ts`, `STATE.md`, `docs/spec-packs/products/maya/STATUS.md` |
